...
if (!function_exists('kt_docdirect_get_everage_rating')) {
	...
	function kt_docdirect_get_everage_rating( $user_id='', $type=false ){
		...
		$list_data = [];
	   	...
	   	$vkl = ['recommendation', 'supporting_staff', 'waiting_time', 'bedside_manner', 'facilities', 'accessibility'];
		...
		while($query->have_posts()) : $query->the_post();
            ...
            foreach ($vkl as $name_type) {
            	$list_data[$name_type]['total'] = $list_data[$name_type]['total']+$user_rating[$name_type];

				if( $user_rating[$name_type] == 0 ){
            		$list_data[$name_type]['details']['0'] = $list_data[$name_type]['details']['0']+1; 
				} else if( $user_rating[$name_type] == 1 ){
            		$list_data[$name_type]['details']['1'] = $list_data[$name_type]['details']['1']+1;
				} else if( $user_rating[$name_type] == 2 ){
            		$list_data[$name_type]['details']['2'] = $list_data[$name_type]['details']['2']+1;
				} else if( $user_rating[$name_type] == 3 ){
            		$list_data[$name_type]['details']['3'] = $list_data[$name_type]['details']['3']+1;
				}
            }

            if ($type == false) {

            	$total += array_sum($user_rating);
			...
			if( $user_rating == 0 ){
				...
				$rate_0['rating']   = $rate_0['rating']+$user_rating;   

				$rate_0['total']	= $rate_0['total']+ 1;   
